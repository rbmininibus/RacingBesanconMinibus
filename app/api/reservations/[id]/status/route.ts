...
export async function PATCH(
  ...
  const { statut, minibus_id } = await req.json()
  const { id } = params
  ...
  const updateData: { statut: string; minibus_id?: string } = { statut }
  if (minibus_id) updateData.minibus_id = minibus_id

  const { error } = await supabase
    .from('reservations').update(updateData).eq('id', id)

  if (error) return NextResponse.json({ error: error.message }, { status: 500 })

  const minibusName = (minibus_id || reservation.minibus_id) === 'minibus-1' ? 'Minibus 1'
    : (minibus_id || reservation.minibus_id) === 'minibus-2' ? 'Minibus 2' : 'Minibus 3'
  ...
  try {
    await resend.emails.send({
      ...
      subject: isValidee ? '✅ Réservation confirmée — Racing Besançon' : '❌ Réservation refusée — Racing Besançon',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background: ${isValidee ? '#198754' : '#dc3545'}; padding: 24px; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 24px;">${isValidee ? '✅ Réservation confirmée' : '❌ Réservation refusée'}</h1>
          </div>
          <div style="padding: 32px; background: #f9f9f9;">
            <h2 style="color: #111;">Bonjour ${reservation.prenom} ${reservation.nom},</h2>
            <p>${isValidee ? "Votre réservation a été <strong>validée</strong>." : "Votre réservation a été <strong>refusée</strong>."}</p>
            <div style="background: white; border-radius: 8px; padding: 20px; margin: 20px 0; border-left: 4px solid ${isValidee ? '#198754' : '#dc3545'};">
              <p><strong>🚌 Minibus attribué :</strong> ${minibusName}</p>
